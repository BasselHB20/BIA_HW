...
def process_shipments():
    ...
    try:
        ...
        if not best_solution :
            return jsonify({'success': False, 'error': 'No solution found'}), 500
        ...
        unallocated_shipments_global = unallocated_shipments
...
# process the TSP file and execute genatic algorithm    
@app.route('/city_page', methods=['POST'])
def process_cities():

    # Use try and catch to handle the execptions
    try:
        ...
        #  then execute the genatic algoritm for each plane
